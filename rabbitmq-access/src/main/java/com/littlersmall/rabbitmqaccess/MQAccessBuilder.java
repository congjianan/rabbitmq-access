...
public class MQAccessBuilder {
	private static final Logger log = LoggerFactory.getLogger(MQAccessBuilder.class);
    private ConnectionFactory connectionFactory;

    public MQAccessBuilder(ConnectionFactory con) {
  ...
  // 通配符模式的sender（topic）
    ...
    public <T> MessageConsumer buildMessageConsumer(String exchange, String routingKey, final String queue,
        ...
        //1  buildQueue    （onsume的手工确认模式 -- 首先在queue创建中指定模式）
            ...
            public DetailRes consume() {
                try {
                    ...
                    while (response == null) {
                        ...
                        Thread.sleep(Constants.ONE_SECOND);//consumer失败后等待时间(mils)
                    }
                    ...
                    System.out.println("消费传来的未经转换的消息:"+message);
                    //2
                    @SuppressWarnings("unchecked")
                    T messageBean = (T) messageConverter.fromMessage(message);
                    System.out.println("消费传来的消息序列化转换过为:"+messageBean);
                    //3
                    DetailRes detailRes;

                    try {
                        detailRes = messageProcess.process(messageBean);
                        System.out.println("消费者处理消息结果:"+detailRes.toString());
                    } catch (Exception e) {
